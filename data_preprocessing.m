function [] =  data_preprocessing(search_path_input, patient_number)
...
load([search_path_input DirStr 'patientdata_20150507'])
subj = patient_number;

%% Get fastingBG
load([search_path_input DirStr 'HbA1c_values'])
ind = find(HbA1c(:,1) == 5000+subj);
HbA1c_subject = HbA1c(ind,2);
...
cgm_time = patientdata(subj).cgm_t;
cgm_value = patientdata(subj).cgm_v;
...
%% SMBG - DP3 version
ind = find(isnan(patientdata(subj).smbgvalues)<.5);
SMBG_times_U_sorted = patientdata(subj).cgm_t(ind);
SMBG_values_U_sorted = patientdata(subj).smbgvalues(ind);
...
%% BOLUS
BOLUS_raw = patientdata(subj).bolus; % U
...
BSL_PRF_mUpmin = 1000*(patientdata(subj).basalprofile)/60;
dUprf = 1000*(patientdata(subj).basal-patientdata(subj).basalprofile)/60 + BOLUS*1000/5; %mU/min
avgbasalprofilerate = patientdata(subj).meanbasal;
avgbasalprofilerate_mUpmin = 1000*avgbasalprofilerate/60;
dU = dUprf + (BSL_PRF_mUpmin-avgbasalprofilerate_mUpmin);
BSL_mUpmin = 1000*(patientdata(subj).basal)/60;
BASAL = patientdata(subj).basal;

%% BW
BW = patientdata(subj).bw;
...
save([search_path_input DirStr 'database_for_BROM' num2str(patient_number) '.mat'],...
